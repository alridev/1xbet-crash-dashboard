...
function App() {
  ...
  const [lostThreads, setLostThreads] = useState([])
  const [message, setMessage] = useState('');
  const [connectionStats, setConnectionStats] = useState(false)
  ...
  useEffect(() => {
    ...
    if (ws.current) {
      ws.current.onmessage = async (e) => {
        ...
        if (dataArray[0] == 'INITIAL_threads') {
          setThreads((pre) => ([...pre, ...dataArray[1]]))
          setLiveThreads([...dataArray[1].filter((element) => element.status == 1)])
          setLostThreads([...dataArray[1].filter((element) => element.status == 0)])

        }

        if (dataArray[0] == 'DISCONNECTED') {
          setConnectionStats(false)
        }

        if (dataArray[0] == 'LIVE') {
          setConnectionStats(true)
        }
    ...
    // console.log([...threads.filter((element)=> element.status == 1)])
  }, [isPaused])
  ...
  return (
    <div className="App">
      <span className='status-badge'> <div className="status-spot" style={connectionStats ? {background:'#dc3545'} : null} ></div> {connectionStats ? 'Live' : 'Offline'}</span>
      <div className="brand">
        <svg
      ...
      <div className="thread-summary">
        ...
        <div className="total-value">
          {Math.round(threads[0]?.value + threads[1]?.value + threads[2]?.value)}
          <p>Total value</p>
        </div>
      </div>
      ...
      <div className="threads">
        {threads.map((element) => (
          <div className="thread-box">
            <span style={(element.status == 0) ? {background:'#dc3545'} : null}>{element.status == 1 ? 'Active' : 'Lost'}</span>
            {Math.round(element?.value)}
            <p>{element.thread}</p>
            <p>{element.rounds} rounds</p>
          </div>
        ))}
